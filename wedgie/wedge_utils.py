...
def plot_1D(npz_name, baselines=[]):
    ...
    plt.subplot(G[:, 0:4])
    for i in baselines:
        plt.plot(plot_data['delays'], plot_data['wslices'][i], label=str(round(plot_data['caldata'][3][i],1))+' m')
    plt.xlabel('Delay (ns)')
...
def plot_multi_1D(npz_names, args, baselines=[]):
    ...
    for n in range(len(npz_names)):
        ...
        # plot the data
        for i in baselines:
            plt.plot(plot_data['delays']*factor, wslices[i], label=str(round(plot_data['caldata'][3][i],1))+' m')
        if len(baselines) == 1:
